...
//
//         // Fetch contacts
//         final contactList = await FlutterContact.getContacts(withProperties: true);
//
//         final Set<String> phoneSet = {};
//
//         for (final contact in contactList) {
//           for (final phone in contact.phones) {
//             final number = phone.number.replaceAll(' ', '');
//             if (!phoneSet.contains(number)) {
//               contacts.add({
...
//   }
  Future<void> getContact() async {
    if (contacts.isEmpty) {
      ...
      if (permission.isGranted && await FlutterContacts.requestPermission()) {
        if (mounted) {
          setState(() {
            _isLoading = true;
          });
        ...
        for (final contact in contactList) {
          for (final phone in contact.phones) {
            final number = phone.number.replaceAll(' ', '').replaceAll('-', '');
            if (!phoneSet.contains(number)) {
              contacts.add({
